# -*- coding: utf-8 -*-
"""
Created on Thu Oct 10 16:21:54 2019
...
df = pd.read_excel('data\\SuperStore\\Superstore.xls', header=0,encoding = 'unicode_escape')
...
# pattern, such as sales are always low at the beginning of the year and high at the end of 
# the year. There is always an upward trend within any single year with a couple of low months 
# in the mid of the year.
# We can also visualize our data using a method called time-series decomposition that allows
# us to decompose our time series into three distinct components: trend, seasonality, and 
# noise.
...
# The plot above clearly shows that the sales of furniture is unstable, along with its 
# obvious seasonality.
...
# Our goal here is to use a “grid search” to find the optimal set of parameters that 
# yields the best performance for our model.
for param in pdq:
...
# It is not perfect, however, our model diagnostics suggests that the model residuals are
# near normally distributed.

# Validating forecasts

# To help us understand the accuracy of our forecasts, we compare predicted sales to
# the real sales of the time series, and we set forecasts to start at 2017–01–01 to the 
# end of the data.
pred = results.get_prediction(start=pd.to_datetime('2017-01-01'), dynamic=False) #results.get_prediction
...
# Overall, our forecasts align with the true values very well, showing an upward trend 
# starts from the beginning of the year and captured the seasonality toward the end of the
# year.
...
# values and what is estimated. The MSE is a measure of the quality of an estimator — it 
# is always non-negative, and the smaller the MSE, the closer we are to finding the line 
# of best fit.
# Root Mean Square Error (RMSE) tells us that our model was able to forecast the average 
# daily furniture sales in the test set within 151.64 of the real sales. Our furniture 
# daily sales range from around 400 to over 1200. In my opinion, this is a pretty good 
# model so far.

# Producing and visualizing forecasts
pred_uc = results.get_forecast(steps=100) #results.get_forecast
...
# Our model clearly captured furniture sales seasonality. As we forecast further out into the
# future, it is natural for us to become less confident in our values. This is reflected by
# the confidence intervals generated by our model, which grow larger as we move further out 
# into the future.
# The above time series analysis for furniture makes me curious about other categories, and
# how do they compare with each other over time. Therefore, we are going to compare time
